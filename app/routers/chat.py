...
async def ask_ai(request: schemas.ChatRequest):
    ...
    if request.child_age is not None:
        ...
        gender_en = "boy" if request.child_gender == "boy" else "girl"
        ...
        child_info_en = f"Child info: age - {request.child_age}, gender - {gender_en}, parent's goal - '{request.parent_goal}'."
    ...
    if request.language == 'kz':
        ...
        system_prompt = f"""You are a professional speech therapist in the 'Soile AI' mobile app. 
        {child_info_en}
        Your goal is to give short, clear, and friendly advice to parents. 
        Respond in English. Keep it brief, maximum 3-4 sentences. Always use emojis."""
